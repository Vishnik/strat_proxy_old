...
class CredentialRewriter {
	...
	#handlePullNotifyMessage(client, message) {
		this._log.log('handle notify message, diff = ' + client.difficulty);
	...
	#handleNotifyMessage(message, rewriteLog, client) {
		if (Math.random() < 0.02) {
	...
	#sendSyntheticWorkerDifficulty()
	{
		// TODO get difficulty from worker
		const difficulty = 65536;
		const message = {
			id:null,
